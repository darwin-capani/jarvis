# DARWIN HUD — Phase-2 Design & Engineering Spec
...
- Target hardware: **any Apple Silicon Mac (M1 or later)**. Developed on an Apple M1 Pro (MacBookPro18,1, 16 GiB), which runs the full scene at 60 fps; the 120 fps figures below are **targets/estimates for ProMotion-class hardware**, not numbers measured on this machine.
...
- One instanced draw: 12 k particles measured on the M1 Pro dev machine (target ~30 k on ProMotion-class / higher-bandwidth hardware), point sprites with depth-faded alpha.
...
| `daemon.started` (system) | `root`, `cloud_key_present` | "DARWIND ONLINE" boot stinger; clears any stale-connection banner |
...
A `panel`-class glass surface (chord `⌥⌘,`) for the runtime knobs that should not require editing `config/darwin.toml` over SSH. Settings the daemon owns are applied over a daemon-side channel, not written by the HUD directly — the HUD stays a client.
...
- **Storage: macOS Keychain, never plaintext.** The HUD writes the key as a generic password item (service `com.darwin.daemon`, account `anthropic_api_key`) via the Security framework; it is never written to disk, config, logs, or telemetry. The panel renders only presence + last-4 characters.
...
**wgpu fallback trigger** (decided at the M3 gate, not re-litigated before): sustained frame time > 8.3 ms on ProMotion-class hardware (the 120 fps target) at full scene after the M3 optimization pass, or input-to-photon latency > 2 frames on the fullscreen surface class. To keep the fallback cheap, the telemetry client, event→state machine, and panel/topic registry live in a renderer-agnostic core (TypeScript module with no three.js imports; its logic mirrors a future `hud-core` Rust crate) — a renderer swap rewrites drawing, not behavior.

### 6.1 Performance budget

120 fps on ProMotion-class hardware = **8.3 ms/frame**. Allocations:
...
|---|---|---|
| M0 | **Shell spike** — Tauri 2 app: fullscreen always-on-top, Dock/menu bar hidden, WS client connected, frame-time overlay | 120 Hz rAF confirmed on ProMotion-class hardware (frame-time histogram); ⌥⌘Q quits; survives daemon restart (auto-reconnect with backoff) |
...
| M2 | **Telemetry overlays** — full §3 event map: transcript feed, intent chips, latency ribbon vs targets, sparkline, memory ticker, status bar, offline banner | Live session against a real `darwind`: every emitted event visibly accounted for; unknown-event injection does not throw |
| M3 | **Particle field + perf gate** — volumetric field, bloom, polish pass to the §6.1 budget | 8.3 ms p95 frame time on ProMotion-class hardware at full scene; ladder verified by forced load; **go/no-go on wgpu fallback decided here** |
...
| M5 | **Deployment** — `com.darwin.hud` LaunchAgent, boot-to-HUD on the dev machine, 24 h soak | Power-on → HUD with no interaction; no memory growth > 5% over soak; HUD kill/relaunch leaves pipeline unaffected |
