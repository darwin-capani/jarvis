...
- Self-heal **v2 has landed** (`daemon/src/heal.rs`, gated **OFF** by default, `mode = "propose"`): error-burst detection → root-cause diagnosis (with the cited source attached) → N Opus candidate diffs → per-candidate staged `cargo check`+`cargo test` → adversarial review + confidence → proposal for gated human apply (`scripts/apply_heal.sh <ts>`); the HUD surfaces it in a warn-amber SELF-REPAIR panel. The full loop is battle-tested through the real cloud by `jarvisd --heal-drill` against a planted fault in a throwaway crate. **Remaining for this phase:** soak-test in propose mode against real bursts, then consider flipping `[self_heal] enabled = true` (auto mode stays opt-in and documented dangerous). See `docs/ARCHITECTURE.md` § Self-heal v2.
...
**Still to build** — the other four launch apps, each against its SPEC.md (spec-only manifests today):

- **Nexus** (`apps/nexus/SPEC.md`) — CoreAudio aggregate-device routing matrix, AUv3 hosting, SM7dB input chain, sub-10 ms monitor path, spectral + LUFS telemetry.
...
- **Silicon Canvas** (`apps/silicon-canvas/SPEC.md`) — wgpu schematic/PCB renderer, KiCad import, net tracing, ERC overlay, 60 fps on 10k+ component boards.
