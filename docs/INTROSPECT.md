...
- **Endpoint Security seam (classifier shipped, live client deferred)** — the pure, CI-tested brain an ES NOTIFY client would drive is built now: `SecurityEvent` (mprotect-exec / MAP_JIT / GET_TASK / signal) → `classify_security_event` (a `jit=false` app making memory executable = W^X violation; a `GET_TASK` = attach/inject signal) → `ingest_security_event` → `introspect.security_event` telemetry + HUD finding. Only the ES *syscall front-end* is deferred (below).

Deferred (with the honest cost recorded below): the **live** Endpoint Security NOTIFY client that feeds the seam above, and the tamper-resistant out-of-process `task_for_pid`/Mach-port corroboration of the cooperative module report.
...
**Deferred, with cost:** Endpoint Security (restricted entitlement + FDA + notarized root host); and the tamper-resistant out-of-process module corroboration via `task_for_pid`/`mach2` (debugger entitlement, useless against hardened own-processes). Neither is required for the shipped self-diagnostics. The live `dyld_report.py` stub and the `sysinfo` sampling are device-gated (the stub is verified on-device; the daemon-side cores are unit-tested).
