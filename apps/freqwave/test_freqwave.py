...
# A wave.solve op carrying a request `id` is answered with a type:"result" line
# echoing that id; an op without one keeps the legacy uncorrelated type:"items"
# line. compute() is pure, so a minimal valid payload (frequency = c -> wavelength
# 1 m) drives handle() straight through reply_result — the reply SHAPE is asserted.
...
def test_non_string_or_empty_id_is_treated_as_absent():
    for bad_id in (7, "", None, ["x"]):
        conn = FakeConn()
        _frame_mod.handle(conn, {"type": "wave.solve", "id": bad_id, "frequency": 299792458})
