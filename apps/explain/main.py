#!/usr/bin/env python3
"""On-device AI code explainer: explain a code snippet in plain English via the local LLM."""
import json
import os
import socket
import sys
...
APP_NAME = os.environ.get("JARVIS_APP_NAME", "explain")
...
def build_prompt(payload):
    ...
    Reads payload["code"] (required, non-empty str) and optional
    payload["language"] (str, used only to tag the fenced code block). Produces a
    tight instruction telling the small local model to explain, in plain English
    and concisely, what the code does."""
    ...
    lang_tag = language.strip() if isinstance(language, str) and language.strip() else ""

    lang_phrase = f" {lang_tag}" if lang_tag else ""
    fence = f"```{lang_tag}\n" if lang_tag else "```\n"

    return (
        "You are a precise code explainer. Explain in plain English what the "
        f"following{lang_phrase} code does. Be concise and accurate: describe its "
        "purpose, inputs, and outputs in 1-4 sentences. Do not restate the code "
        "line by line, do not add commentary, and do not include any code in your "
        "answer.\n\n"
        f"{fence}{code}\n```\n\n"
        "Explanation:"
    )
...
def handle(conn, msg):
    ...
    if op == "start":
        send(conn, {"type": "status", "data": {"tool": "explain.run", "ready": True}})
    elif op == "refresh":
