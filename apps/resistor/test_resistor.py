...
import json  # noqa: E402 — mid-file, after the app's own imports
...
def test_tool_op_with_id_answers_a_correlated_result():
    conn = FakeConn()
    _frame_mod.handle(conn, {"type": "resistor.decode", "id": "req-7",
...
def test_non_string_or_empty_id_is_treated_as_absent():
    for bad_id in (7, "", None, ["x"]):
        conn = FakeConn()
        _frame_mod.handle(conn, {"type": "resistor.decode", "id": bad_id,
                                 "bands": ["yellow", "violet", "red", "gold"]})
