...
# textkit.stats is the declared tool: a request carrying a non-empty string `id`
# is answered with a type:"result" line echoing that id; an op without one keeps
# the legacy uncorrelated type:"items" line (voice/refresh paths depend on it).
...
def test_non_string_or_empty_id_is_treated_as_absent():
    for bad_id in (7, "", None, ["x"]):
        conn = FakeConn()
        _frame_mod.handle(conn, {"type": "textkit.stats", "id": bad_id, "text": "hi there"})
